...
const BlogPostCard:React.FC<CardBlogProps> = ({title, first=false, image, badgeTitle, href="#"}) => {
    const sizeText = first ? 'text-3xl' : 'text-xl';

    const path = `/blog/${href}`;

    return (
        <DaddusLink href={href} className="relative bg-cover bg-center h-full w-full rounded-lg overflow-hidden shadow-md" variant={"ghost"} isTagAnchor>
            <Image
                src={image.src}
                alt={image.alt}
                layout="fill"
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-4 lg:p-10 text-white">
                ...
                <h2 className={`text-sm md:text-xl lg:${sizeText} font-bold whitespace-normal`}>{title}</h2>
            </div>
        </DaddusLink> 
    )
}
