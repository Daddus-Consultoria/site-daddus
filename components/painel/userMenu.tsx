      ...
      {open && (
        <div className="absolute right-0 z-20 mt-2 w-56 rounded-xl border border-gray-200 bg-white p-2 shadow-lg">
          <div className="border-b border-gray-100 px-3 py-2">
            ...
            {user.role && <p className="mt-1 text-xs font-semibold text-primary">{"data" in user.role ? user.role.data?.attributes?.name : "attributes" in user.role && user.role.attributes ? user.role.attributes.name : (user.role as { name?: string }).name}</p>}
          </div>
